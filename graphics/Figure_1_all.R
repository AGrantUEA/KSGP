...
pdf("final/figure_1_all.pdf",width=16,height=8) 
...
for (j in 1:n){
  ...
  if (toplot$type[j]==1){
    ...
    if (j==1){
      ...
      parameter<-plot (nnn,pct,type="l",lty=toplot$lty[j],col=toplot$colour[j],lwd=3,ylim=c(75.,100.),ylab="Similarity of best match (%)", xlab="Cumulative percentage of OTUs/Sequences")
    }else {
...
legend(-3,84,legend=toplot$caption,col=toplot$colour,lwd=3,lty=toplot$lty,cex=1.0,bty="n")


toplot=read.csv("cumulative_plots_b.csv")
toplot
n=length(toplot$file)
load("AR_keep_v2_phyloseq.Rdata")

counts=rowSums(physeq@otu_table)
tokeep=rownames(physeq@tax_table[which(physeq@tax_table[,1]!="?"),1])
...
for (j in 1:n){
  ...
  if (toplot$type[j]==1){
    data=read.table(toplot$file[j],header = FALSE,sep="\t")
    data=data[which(data$V9 %in% tokeep),]
    pct=sort(as.numeric(data$V4),decreasing=TRUE,na.last=TRUE)
  ...
  if (toplot$type[j]==2){
    ...
    data=read.table(toplot$file[j],header = FALSE,sep="\t")
    data=data[which(data$V1 %in% tokeep),]
